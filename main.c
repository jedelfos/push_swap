...
int     skip_word(char *str, int i)
{
    ...
    if ((!(str[i] == '+' || str[i] == '-' || (str[i] >= '0' && str[i] <= '9'))) || str[i] == '\0')
        return(-1);
    return(i);
}
...
int 	check_argv(int argc, char **argv)
{
	...
	while(argc > i)
	{
		while (argv[i][y] != '\0')
		{
			if (!(((argv[i][y] >= '0' && argv[i][y] <= '9') || (argv[i][y] == '\t' || argv[i][y] == '\n' || argv[i][y] == '\v' || argv[i][y] == '\f' || argv[i][y] == '\r' || argv[i][y] == ' ' || argv[i][y] == '+' || argv[i][y] == '-'))))
				error(1);
			y++;
		}
		y = 0;
...
int main(int argc, char **argv)
{
    ...
    while (list[0]->first->next != NULL)
    {
        ...
        positionement(list, place);
 		//-affichage(list);
		//-printf("la >> %i  %i \n", list[0]->first->val , list[1]->n_val);
//		if (is_sort(list) && list[0]->first->val >= list[1]->n_val)
//			break;
    }
//	if (!(is_sort(list) && list[0]->first->val >= list[1]->n_val))
//printf("\ntest\n\n");
